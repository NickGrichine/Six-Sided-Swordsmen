...
public static class HexPathfinder
{
    public static List<Tile> FindPath(Tile start, Tile goal)
    {
        ...
        {
            Debug.LogWarning($"Pathfinding failed: goal tile '{goal.name}' cannot be entered.");
            return new List<Tile>();
        }
    ...
    private static List<Tile> ReconstructPath(Dictionary<Tile, Tile> cameFrom, Tile goal)
    {
        ...
        {
            path.Add(step);
            step = cameFrom.TryGetValue(step, out Tile previous) ? previous : null;
        }

        path.Reverse();
        return path.Count > 1 ? path : new List<Tile>();
    }
}